...
class AllListView(ListView):
    ...
    def get_queryset(self):
        ...
        free_list = Free.objects.order_by('-hits') 
...
# 카테고리 음악방송만 보기
class BroadcastingListView(AllListView):
    def get_queryset(self):
        search_keyword = self.request.GET.get('q', '')
        search_type = self.request.GET.get('type', '')
        free_list = Free.objects.filter(category2='음악방송').order_by('-id')
...
# 카테고리 뮤직비디오만 보기
class MusicvideoListView(AllListView):
    def get_queryset(self):
        search_keyword = self.request.GET.get('q', '')
        search_type = self.request.GET.get('type', '')
        free_list = Free.objects.filter(category2='뮤직비디오').order_by('-id')
...
# 카테고리 공연만 보기
class ShowListView(AllListView):
    def get_queryset(self):
        search_keyword = self.request.GET.get('q', '')
        search_type = self.request.GET.get('type', '')
        free_list = Free.objects.filter(category2='공연').order_by('-id')
...
# 카테고리 기타만 보기
class EtcListView(AllListView):
    def get_queryset(self):
        search_keyword = self.request.GET.get('q', '')
        search_type = self.request.GET.get('type', '')
        free_list = Free.objects.filter(category2='기타').order_by('-id')
